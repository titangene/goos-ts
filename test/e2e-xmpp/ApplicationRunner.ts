...
// 完整獨立複製一份 test/e2e/ApplicationRunner.ts 的骨架（不 import），但
// startSniper()/stop() 的實作方式跟 Redis 版完全不同：Redis 版整個測試
// 期間共用同一個由 Playwright webServer 管理的長駐 server；這裡比照 Java
// 版 ApplicationRunner.java 的 startSniper()（`new Thread(...) { run() {
