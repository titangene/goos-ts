...
// 互不取代）。跟 production code 共用同一套 XMPPConnection/XMPPChatManager
// 抽象（見 docs/xmpp-ts-vs-java-differences.md），用
...
// 這個檔案放在 test/e2e-xmpp/，同時被 test/e2e-xmpp/*.test.ts（Playwright）
// 與 test/integration/xmpp/XMPPAuctionHouse.test.ts（Vitest）匯入使用，
// 底下用的 vitest 的 expect() 已實測驗證在 Playwright test runner 內一樣能
// 正常運作（不依賴 vitest 的執行環境，是獨立可用的斷言函式）。
export class FakeAuctionServer {
  ...
  async startSellingItem(): Promise<void> {
    ...
    // async factory（見 docs/xmpp-ts-vs-java-differences.md），這裡因此
