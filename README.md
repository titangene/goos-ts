...
| 拍賣訊息協定      | XMPP（Smack library，需搭配 Openfire server）                           | Redis Pub/Sub（`redis` client，見 [ADR-0002](docs/adr/ADR-0002-mqtt-replaces-redis.md)）                                                                 |
...
| 整合測試          | `test/integration`（Swing `MainWindow`、XMPP `XMPPAuctionHouse`）       | `test/integration`（Nuxt app、Redis `RedisAuctionHouse`；`test/integration/mqtt` 仍保留），另加 `test/integration/app`                                   |
| End-to-end 測試   | `test/end-to-end`（`ApplicationRunner` + WindowLicker 驅動 Swing 元件） | `test/e2e`（Playwright 驅動瀏覽器）                                                                                                                      |
| 假拍賣現場        | `FakeAuctionServer.java`（連 XMPP）                                     | `test/e2e/RedisFakeAuctionServer.ts` + `tools/fake-auction.ts`（連 Redis；MQTT 版仍保留於 `tools/fake-auction-mqtt.ts`）                                 |
| CI 測試用依賴服務 | Openfire（XMPP server）                                                 | Redis（GitHub Actions `services:` 啟動）；Mosquitto 仍保留啟動供 `test/integration/mqtt` 使用，見 [ADR-0005](docs/adr/ADR-0005-ci-local-dev-workflow.md) |
...
| 訊息協定實作         | `src/auctionsniper/xmpp/*.java`（`XMPPAuction`、`XMPPAuctionHouse`、`AuctionMessageTranslator`…） | `server/auctionsniper/redis/*.ts`（`RedisAuction`、`RedisAuctionHouse`、`AuctionMessageTranslator`…，逐檔對照 Java 版，見 [`docs/differences-from-java.md`](docs/differences-from-java.md) 與 [`docs/redis-vs-mqtt-implementation.md`](docs/redis-vs-mqtt-implementation.md)；`server/auctionsniper/mqtt/*.ts` 仍保留在 repo 中，但目前未被 app 使用） |
...
npm run test:integration    # 整合測試（接真實 Redis；test/integration/mqtt 需要真實 Mosquitto）
