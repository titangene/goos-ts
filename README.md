...
Auction Sniper — TypeScript + Nuxt + XMPP + WebSocket 版本，改寫自 [Growing Object-Oriented Software, Guided by Tests](https://www.growing-object-oriented-software.com/) 書中的 Java 範例。
...
| 整合測試          | `test/integration`（Swing `MainWindow`、XMPP `XMPPAuctionHouse`）       | `test/integration`（Nuxt app、XMPP `XMPPAuctionHouse`），另加 `test/integration/app`                                                                                 |
| End-to-end 測試   | `test/end-to-end`（`ApplicationRunner` + WindowLicker 驅動 Swing 元件） | `test/e2e`（Playwright 驅動瀏覽器）                                                                                                                                  |
| 假拍賣現場        | `FakeAuctionServer.java`（連 XMPP）                                     | `test/e2e/FakeAuctionServer.ts` + `tools/fake-auction.ts`（連 Prosody）                                                                                              |
| CI 測試用依賴服務 | Openfire（XMPP server）                                                 | Prosody（GitHub Actions 內 build/run `docker/xmpp/` image）                                                                                                          |
...
| 訊息協定實作         | `src/auctionsniper/xmpp/*.java`（`XMPPAuction`、`XMPPAuctionHouse`、`AuctionMessageTranslator`…） | `server/auctionsniper/xmpp/*.ts`（`XMPPAuction`、`XMPPAuctionHouse`、`AuctionMessageTranslator`…，逐檔對照 Java 版，見 [`docs/differences-from-java.md`](docs/differences-from-java.md)）                  |
...
- Node.js
- Prosody（XMPP server，本機或 Docker 皆可）
...
npm run test:integration    # 整合測試（接真實 Prosody）
npm run test:integration:app # UI 元件測試
npm run test:e2e            # e2e 測試（Playwright，需要真實 Prosody；需先 npm run build）
npm test                    # 全部跑一遍（含 build）
```
...
```bash
npm run fake-auction -- item-54321          # 連本機 Prosody
npm run fake-auction:remote -- item-54321   # 連 XMPP_SERVICE_URL/XMPP_DOMAIN 指定的 Prosody（.env.local）
```
