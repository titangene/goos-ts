...
- main 分支已有 [`npm run fake-auction:remote`](../tools/fake-auction.ts)（見 [`docs/fake-auction.md`：步驟](fake-auction.md#步驟) 第 2 步），可以手動連已部署的 Prosody；但還沒有 poc 分支已有的 `test/integration` 測試層，因此暫時沒有對應的「SASL 登入自動化驗證」章節，等 main 分支的 TDD 進度補上對應功能後再補充。
